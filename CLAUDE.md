...
PedestrianSimulator (pysocialforce; Ego車両を斥力源として歩行者が回避)
  ...
  → FailSafeStateMachine (src/core/state_machine.py; NORMAL→CAUTION→EMERGENCY で
     加速度・曲率制約を段階的に緩和、状態昇格ごとに同一ステップ内で再計画
     （実質最大2回/ステップ、カウンタ上限3はバックストップ）、最終的に緊急停止)
```
...
- **設定**: `src/config/__init__.py` の `SimulationConfig` dataclass が**唯一の情報源**（default_config.yaml は廃止済み）。`load_config()` はシナリオ YAML のキーで dataclass デフォルトを上書きし、`validate_config()` で読み込み時に検証する。設定キーを追加する際は dataclass とバリデーションの2箇所を揃える。
