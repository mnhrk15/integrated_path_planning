...
def run_one(scenario: str, condition: str, method: str, seed: int) -> dict:
    set_seed(seed)
...
def build_report(df: pd.DataFrame, outdir: Path, repo: Path):
    ...
    lines += [
        ...
        "",
        "| scenario | method | Δtime [s] | p(time) | Δrect clearance [m] | p(clear) | rect viol (c→m) | legacy MinDist (c→m) |",
        "|---|---|---|---|---|---|---|---|",
    ]
    ...
    if not args.report_only:
        ...
        for i, (scenario, condition, method, seed) in enumerate(jobs, 1):
            ...
            print(f"[{i}/{len(jobs)}] {Path(scenario).stem} {condition} {method} seed={seed}",
                  flush=True)
            try:
                row = run_one(scenario, condition, method, seed)
