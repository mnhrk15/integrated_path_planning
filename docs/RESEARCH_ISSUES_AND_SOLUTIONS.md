...
**実施結果（2026-06-10、★2 後半完了 = v0 per-agent ランダム化 + KDE-NLL 併記。計306ラン: 決定論 GT 123ラン `output/benchmark_nll_s{1,2,3}` + ランダム化 GT 183ラン `output/benchmark_rand_s{1,2,3}`）:**

- 実装: (i) KDE-NLL — 予測サンプル分布に対する GT の負対数尤度（Scott バンド幅・floor 0.05 m・per-point −20 nats クリップ = Trajectron++ 系の慣行。CV の「決定論予測の複製」疑似分布は除外し NaN）。(ii) `sfm_v0_randomization` — per-agent N(0, 0.19) ノイズ（Moussaïd 2009 の個体間 SD）を希望速度に付加、`sfm_v0_min`=0.3 で下限。pysocialforce の state setter が毎ステップ max_speeds を再計算する罠があるため、永続する initial_speeds 経由で適用（回帰テスト付き）。ランダム化 GT 下では CV も確率的になるため20シード実行（**A-3 の n=1 非対称性を同時解消**）。挙動保存: rand off は `comfort_s*` と全123ラン・全共有列でビット一致。
- **主結果1（A-1 の核心）: GT 分布化で深層同士の ADE 序列が縮小・消失する。** S3 の SGAN 優位（det: 2.13 vs LSTM 2.25、p=1.5e-34）はランダム化で**消失**（LSTM 2.02 vs SGAN 2.05、p=0.61）。S2 の LSTM 優位も p=5.4e-32→0.038 へ縮小。S1 は LSTM 優位が残存（p=1.2e-5）。S3 の「SGAN の相互作用考慮の利点」と読めた差は、決定論的 SFM への適合が主因だったことを示す。
- **主結果2: deep > CV の best-of-N ADE 優位は分布化後も生存**（全シナリオで LSTM/SGAN が CV に有意勝ち、p≤0.011）。一方 **P-ADE は分布化後も全シナリオ CV が最小** — 「best-of-N でのみ深層が優位」という A-1 の構図は GT 分布化に対して頑健。
- **主結果3（副産物、A-2 に直結）: S2 で本プロジェクト初の衝突が発生。** ランダム化 GT 下の S2 で 31/60 ランに1ステップの graze（中心間 1.12–1.20 m、従来定義の衝突閾値 1.2 m をわずかに割る）。**CV 10/20・LSTM 10/20・SGAN 11/20 と手法非依存**で、ほぼ同一のシード集合（v0 抽選は手法間で共有される設計）に集中 → 速度異質性が作る「困難構成」はどの予測手法でも回避されず、**衝突ゼロの天井は GT の決定論性が作っていた**ことの直接証拠。S1/S3 は分布化後も衝突ゼロ。A-2 の高ストレスシナリオ設計には v0 ランダム化の組み込みが最小コストの近道。
- NLL: 全条件で LSTM ≤ SGAN（det: 全シナリオ p<6e-12。rand: S1 p=1.7e-8、S2 p=0.30、S3 p=0.019）。絶対値は 16.2–19.1 nats とクリップ上限 20 の近傍 = **両モデルとも GT の大半をサンプル支持域外に置く重度の過信**（B-1 ドメインギャップの定量化であり、較正評価・conformal マージン（B-1 解決策3 / ★5）の動機付け）。GT 分布化で NLL は全セル改善（カバレッジ増）するが水準は変わらない。
- 限界: det↔rand の条件間 ADE 比較は endogeneity により不可（A-4 課題4 と同種の注意。手法間比較は各条件内で完結）。分布化は v0 のみで、τ・相互作用パラメータの異質性は未導入（Seer 2014 の個体別分布が取得できれば拡張可能）。NLL の絶対値はバンド幅 floor とクリップに依存する指標であり、列内（手法間）比較に限定して使う。
...
- ★2 **完了**（前半: single-sample P-ADE 表併記。後半: 出典調査 → v0 per-agent ランダム化 + KDE-NLL、A-1 の実施結果ブロック2件参照）。主発見: GT 分布化で S3 の SGAN ADE 優位が消失、S2 で初の衝突（手法非依存、31/60ラン）が発生し A-2 の天井が破れた。A-3 の CV n=1 非対称性もランダム化 GT 下で解消。
