...
- **実験1（膨張マージン対照）の結論: 利得は保守性だけでは説明できない。** どの inflation も3シナリオ同時に「MinDist ≥ robust(ε=0) かつ Time ≤ robust」を達成できず（平均ベース）、全 inflation が少なくとも1シナリオで robust に有意に劣る（有意性ベース、p<0.05）。S2/S3 では robust がトレードオフ曲線を支配: robust と同等の MinDist に到達するには inflation 1.35〜1.5 が必要だが、その走行時間コストは +1.8〜+2.8 s（robust は +0.05〜+1.0 s）。S1 のみ inflation で MinDist は robust を超えるが、有意な Time 増（+0.4〜+0.7 s、かつ 16〜19/20 ランがゴール未到達で右側打切り）を伴うトレードオフであり支配ではない。
...
- 全480ランで衝突ゼロ。ADE は robust 化で統計的に実質不変（SGAN は全シナリオ p>0.15。LSTM は S1/S3 でむしろ僅かに改善、ego 挙動変化→歩行者反応の波及による副次効果）。
