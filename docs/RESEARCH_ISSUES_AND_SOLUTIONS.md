...
- **副産物（重要）: 論文の `statistical_benchmark` CSV は現行コードを再現しない**（ADE max|Δ|≈0.6〜0.8、S2 では time max|Δ|=10.7 s）。原因はコミット 715d7b3（ego-歩行者斥力修正 + ADE/FDE 再設計）以前の生成物であるため。**ジャーナル化時は論文の全表を現行コードで再生成する必要がある**。
