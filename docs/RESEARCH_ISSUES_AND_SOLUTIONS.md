...
5. **検証の負債**: (i) ★1（chance-constrained の分布価値）の結論は単一円幾何で導出されており multi-circle 下での頑健性が未確認 → `run_da_poc.py` に `ego_footprint` オーバーライド追加で再検証可能（小コスト）。(ii) 判定点3倍 × 分布判定20サンプルのリアルタイム性（C-3 と同根）が未実測 → proc_planning の既存記録機構で1ラン実測すれば足りる。
...
- ★3 **完了**（A-4 の検証結果・再検証ベンチマーク結果ブロック参照、右側打切り解消済み）。フォローアップ5件は A-4 の「課題と対応案」に整理。5円化（課題1）と状態マシンの clearance 化（課題3、挙動不変を369ランで実証・jerk 仮説は棄却）は実施済み。残りは ★1 の multi-circle 再検証（課題5(i)）とリアルタイム実測（課題5(ii)）。
