...
> **来歴メモ（2026-06-23 追記）**: 本書は AVEC 提出時点の記録。所見（表・統計・p値）はすべて本文内に自己完結している。
> 本文が参照する raw キャンペーン成果物（`output/*_g5a2/`・`output/*_final/` など）と旧セッション文書（`HANDOVER_*` 等）は
> 提出後のディレクトリ整理で削除済み — 前者はベンチマークスクリプトで再生成可能、後者は git 履歴に残存（コミットハッシュ参照は有効）。
...
- **エンベロープはゼロラグ（同ステップ）clearance を消費してはならない**: 実装→実験で棄却済み（S1 SGAN 3/5 シード stall→timeout、S3 cv limit cycle）。1ステップ遅れ消費が較正済み仕様（コミット `05f3e41`。当時の詳細メモ HANDOVER_S5 は整理削除済み・git 履歴に残存）。
- 前方 clearance（clearance_ahead）必須・S2 エンベロープ無効・S1 回復ゲート 2.0 は従来どおり（同上）。

---

## 9. ランダム化 GT での robust 検証（`exp_robust_rand_g5a2`、2026-06-12 S8、rebuttal 用）

想定査読質問「robust の利得検証が決定論的 GT のみ」への回答データ。`run_da_poc.py` に `--v0-randomization` フラグを追加（`src/` 不変更）し、Table 2 と同一のランダム化 GT（seeds 0–19）で 4 条件 × 3 シナリオ × 20 シード = **240 ラン完走（FAILED 0）**。single 条件は `benchmark_rand_*_g5a2` と **per-seed 一致**（ハーネス等価性を実証、差分は計画器のみに帰属）。

**確定知見**（詳細・rebuttal 用英文は当時の `output/exp_robust_rand_g5a2/REPORT.md`＝提出後に整理削除・再生成可能）:
1. **robust(ε=0) はランダム化 GT でも全 120 ラン衝突ゼロ**（single の残存衝突 SGAN 0/2/1・LSTM 0/1/1 を消去。CV は 4/1/0）。Fisher: CV 5/60 vs robust 両系統 0/120 で p=0.0037。
2. **MinDist/MinTTC は全シナリオ・両系統で有意増**（ΔMinDist: SGAN +0.34〜+0.84、LSTM +0.52〜+1.07 m、全て p≤1.1e-3）。
3. **時間コストなし**: S2 で有意短縮（−2.8/−3.8 s）、S1/S3 非有意。**タイムアウトは robust の方が少ない**（S1: 3→1/3→2）— 決定論側 ★1 の打切り注記はランダム化 GT では逆方向で、隠れコスト懸念なし。
4. 論文は不変更（rebuttal 弾薬として保持。利得が LSTM でも生じる構図も決定論側と同一）。

## 付録: 生成物の所在（当時の構成記録）

> 以下の raw キャンペーン成果物は論文提出後のディレクトリ整理で `output/` から削除済み（gitignore 対象の使い捨てシンクのため git 履歴にも無い）。
...
- 検証ゲート記録・修正全リスト: HANDOVER_S3 §3〜4（整理削除済み・git 履歴に残存）
