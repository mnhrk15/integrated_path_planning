...
11. [プランナ高速化 (v3.7)](#11-プランナ高速化-v37)
...
Frenetプランナの候補生成と衝突判定を高速化し、リアルタイム性を向上させました。

### ベクトル化とバッチ生成
- **ポリノミアルのベクトル評価**: 時間配列 `t` を一括生成し、縦/横の軌道をNumPyでまとめて計算
- **係数計算の再利用**: 各 `Ti` で行列逆行列をキャッシュし、軌道係数の計算をバッチ化
- **候補生成のバッチ処理**: `target_speed` と `lateral_offset` の組み合わせをまとめて作成し、ループ回数を削減

### 衝突判定の前段フィルタ
- **AABBフィルタ**: 経路の外接矩形と障害物の外接矩形を用いて、不要な距離計算をスキップ
- **動的障害物にも適用**: 障害物軌跡のAABBで候補を絞り、時間整合距離計算の対象を削減

### 効果
- プランナ候補生成の計算負荷を削減
- 衝突判定の無駄な距離計算を抑制し、リアルタイム性を改善
